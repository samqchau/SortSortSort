...
function selectionSort(arr) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    animations.push(function () {
      childNodesArray[i].style.backgroundColor = red;
    });
    for (let j = i + 1; j < len; j++) {
      ...
      if (arr[j] < arr[newMinIndex]) {
        if (newMinIndex !== i) {
          ...
          colorChange = j;
          animations.push(function () {
            childNodesArray[colorChange].style.backgroundColor = red;
          });
        }
        //newMinIndex = j;
        minIndex = j;
    ...
    if (minIndex !== i) {
      for (let i = 0; i < 100; i++) {
        animations.push(function () {
          childNodesArray[minIndex].style.backgroundColor = green;
        });
      }
      temp = arr[i];
      arr[i] = arr[minIndex];
    ...
    for (let j = 0; j < len - 1 - k; j++) {
      ...
      if (arr[j + 1] < arr[j]) {
        ...
        animations.push(function () {
          ...
          childNodesArray[j].style.height = `${
            childNodesArray[j + 1].style.height
          }`;
...
function insertionSort(arr) {
  ...
  for (let i = 1; i < len; i++) {
    ...
    for (let o = i - 1; o > j; o--) {
      animations.push(function () {
        childNodesArray[o].style.backgroundColor = purple;
      });
    }
